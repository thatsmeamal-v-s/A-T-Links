...
const EditContact = (props) => {
   let { showPanel, toggleShowPanel, setShowEditPanel, updateEmail, updatePhoneNumber, setEditContactIndex } = props
   ...
   const deleteContact = (deleteContact) => {
      ...
      let newContactList = contactList.filter((item) => item.id !== deleteContact.id)
   ...
   return (
      <div className="sidenav">
         <div className="card-heading">
            <img className="card-header-icon edit-contact-leftarrow" alt="leftarrow" src={leftarrow} onClick={() => togglePanel()} />
            <span>Contact</span>
         ...
         <div className="edit-contact-container">
            {
                  ...
                  return (
                     <div key={item.id} id={index} className="card edit-contact-card">
                        <div className="edit-card-heading">
                           <img className="card-header-icon edit-card-header" alt="contact" src={contact} />
                           <span>{item.name}</span>
                           <img className="edit-header-icon" alt="edit" src={edit} onClick={() => openEditPanel(item, index)} />
                           <img className="delete-header-icon" alt="delete" src={trash} onClick={() => deleteContact(item)} />
                        </div>
                        <div className="edit-card-content-container">
                           <div className="email-content edit-content">
                              ...
                              <span>{item.emails.length === 0 ? 'No Emails' : item.emails.join('/')}</span>
                           </div>
                           <div className="phonenumber-content">
                              <img className="edit-phone-icon" alt="contact" src={phone} />
                              <span>{item.phonenumbers.length === 0 ? 'No Contacts' : item.phonenumbers.join('/')}</span>
                           </div>
                        </div>
                     </div>
                  )              
               ...
               :
               <button type="button" className="add-contact-button" onClick={() => openEditPanel({emails:[], phonenumbers:[]},0)}>
                  <span>Add Contacts</span>
             </button>
            }
         </div>
         <button type="button" className="add-contact-save-button" onClick={() => updateContactList()}>
            <span>Save</span>
         </button>
      </div>
   )
