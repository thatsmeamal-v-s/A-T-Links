...
const AddContact = (props) => {
  ...
  const addNewContact = () => {
    ...
    let filteredEmails = emailList.filter(email => { return email !== "" })
    let filteredPhonenumbers = phoneNumberList.filter(phonenumber => { return phonenumber !== "" })
  ...
  return (
    <div className="sidenav">
      <div className="card-heading">
        <img className="card-header-icon edit-contact-leftarrow" alt="leftarrow" src={leftarrow} onClick={() => backToContacts()} />
        <span>Contacts</span>
      ...
      <div className="add-contact-container">
        <label>Email ID</label>
        {
          emailList.map((email, index) => {
            return (
              <input type="text" key={index} id={index} placeholder="eg: salesteam@br.in" value={email} className="form-control" onChange={(e) => handleEmailChange(e)} />
            );
          })}
        <button type="button" className="add-contact-button" onClick={() => addEmailField()}>
        ...
        <label>Contact Number</label>
        {
          phoneNumberList.map((phoneNumber, index) => {
            return (
              <input type="number" key={index} id={index} placeholder="eg: 8517463218" value={phoneNumber} className="form-control" onChange={(e) => handlePhoneNumberChange(e)} />
            );
          })}
        <button type="button" className="add-contact-button" onClick={() => addContactField()}>
          <img className="plus-icon" alt="plus" src={plus} />
          <span>Add More</span>
        </button>
      </div>
      <button type="button" className="add-contact-save-button" onClick={() => addNewContact()}>
        <span>Save</span>
      </button>
    </div>
  );
